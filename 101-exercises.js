...
function timesTwoPlusThree(number1){
    return number1 * 2 + 3;
}
...
function circumference(number1){
...
function isVowel(vowel){
    ...
    return vowels.indexOf(vowel) + 1 ? true: false;
}
...
function hasVowels(vowel){
    var contains = false;
    var vowels = ['a', 'e', 'i', 'o', 'u', 'A', 'E', 'I', 'O', 'U']
    var vowelArray = vowel.split('');
    vowelArray.forEach (function(element){
        if (vowels.indexOf(element) + 1 != 0){
            contains = true;
        }
    });
    return contains;
...
function removeVowels(vowel){
    var vowels = ['a', 'e', 'i', 'o', 'u', 'A', 'E', 'I', 'O', 'U']
    var vowelArray = vowel.split('');
    vowelArray.forEach (function(element, index){
        if (vowels.indexOf(element) + 1 != 0){
            vowelArray.splice(vowelArray.indexOf(element), 1);
        }
    });
    var newVowel = vowelArray.join('');
    return newVowel;
}
...
function startsWithVowel(string){
    ...
    var bool = false;
    if (vowels.indexOf(vowelArray[0]) + 1 != 0){
        bool = true;
    }
    return bool;
}
...
function endsWithVowel(string){
    var vowels = ['a', 'e', 'i', 'o', 'u', 'A', 'E', 'I', 'O', 'U'];
    var vowelArray = string.split('');
    return vowels.indexOf(vowelArray[vowelArray.lastIndex]) > -1 ? true : false;
}

assert(endsWithVowel("ubuntu"), false, "Exercise 48");
assert(endsWithVowel("banana"), false, "Exercise 48");
assert(endsWithVowel("mango"), false, "Exercise 48");
...
function startsAndEndsWithVowel(string){
    var vowels = ['a', 'e', 'i', 'o', 'u', 'A', 'E', 'I', 'O', 'U'];
    var vowelArray = string.split('');
    return vowels.indexOf(vowelArray[vowelArray.lastIndex]) > -1 || vowels.indexOf(vowelArray[0]) > -1 ? true : false;
}
...
function getSmallestNumber(arg){
    var max = 0;
    var min = 0;
    arg.forEach(function(element){
        if(element > max || max == 0){
...
function onlyPositiveNumbers(arg){
    var bucket = [];
    arg.forEach(function(element){
        if (element > 0){
            bucket.push(element);
...
function onlyNegativeNumbers(arg){
    var bucket = [];
    arg.forEach(function(element){
        if (element < 0){
            bucket.push(element);
...
function onlyPositiveEvens(arg){
    var bucket = [];
    arg.forEach(function(element){
...
function onlyNegativeEvens(arg){
    var bucket = [];
    arg.forEach(function(element){
        if (element % 2 == 0 && element < 0){
            bucket.push(element);
...
function onlyNegativeOdds(arg){
    var bucket = [];
    arg.forEach(function(element){
        if (element < 0 && element % 2 == 1 ||  element % 2 == -1){
            bucket.push(element);
        }
...
function shortestString(arg){
    var bucket = 30;
    var arr = [];
    arg.forEach(function(element){
        if (element.split('').length < bucket){
            bucket = element.split('').length;
            arr = element;
        }
...
function getUniqueValues(arg){
    var bucket = 0;
    var arr = [];
    var arrBucket = [];
    arg.forEach(function(element){
        if (!arrBucket.includes(element))
            arrBucket.push(element)
    });
    // if (element.split('').length > bucket){
    //   bucket = element.split('').length;
    //   arr = element;
...
function totalOfBookPrices (arg){
    var bucket = 0;
    arg.forEach(function (element){
...
function getAverageBookPrice (arg){
    var bucket = 0;
    arg.forEach(function (element){
        bucket += element.price;
    });
...
function numberOfItemTypes (arg){
...
function totalNumberOfItems (arg){
    var bucket = 0;
    arg.items.forEach(function (element, index){
        bucket += element.quantity;
    });
    return bucket;
}
...
function getAverageItemPrice (arg){
    var bucket = 0;
    arg.items.forEach(function (element, index){
        bucket += element.price;
    });
    return bucket / arg.items.length;
}
...
function getAverageSpentPerItem (arg){
    var bucket = 0;
    var finalQ = 0;
    arg.items.forEach(function (element){
        finalQ += element.quantity;
        bucket += (element.price * element.quantity);
    });
    return (bucket / finalQ);
}
...
function mostSpentOnItem (arg){
    var bucket = 0;
    var finalObj = {};
    arg.items.forEach(function (element){
        if (element.price * element.quantity > bucket){
            bucket = (element.price * element.quantity);
            finalObj = element;
        }
    });
