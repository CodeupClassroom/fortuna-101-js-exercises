...
//  Exercise 1
// On the line below, create a variable named on MarsRightNow and assign it the boolean value of false
...
// Exercise 24 - finally fixed this one. Good job bruv! Uvijek guraj!
...
function startsWithVowel(string) {
    var firstLetter = string.charAt(string.length = 0);
...
function startsAndEndsWithVowel(string) {
    var firstAndLastLetter = string.charAt(string.length = 0) + string.charAt(string.length - 1);
    return isVowel(firstAndLastLetter);
}
...
function firstAndLast(sequence){
    return [sequence[0], sequence.splice(-1)[0]];
}
...
function median(nums) {
    var medianAvg = 0;
    if (nums.length % 2 === 1) {
        medianAvg = ((nums.length - 1)/2);
        return nums[medianAvg];
    } else {
        var firstNum = nums[(nums.length/2) - 1];
        var secondNum = nums[(nums.length/2)];
        medianAvg = (firstNum + secondNum)/2;
        return parseFloat(medianAvg.toFixed(1));
    }
}
...
function maxMinusMin(nums) {
    var sorted = nums.sort(function(a,b){return(a-b)});
    var minNumber = sorted[0];
    var maxNumber = sorted[nums.length - 1];
    var maxMinDifference = maxNumber - minNumber;
...
function onlyPositiveNumbers(nums) {
    var positiveNumsArray = [];
    for (var i=0; i < nums.length; i++) {
        if (nums[i] > 0){
            positiveNumsArray.push(nums[i]);
        }
    }
...
function onlyNegativeNumbers(nums) {
    var negativeNumsArray = [];
    for (var i=0; i < nums.length; i++) {
        if (nums[i] < 0) {
            negativeNumsArray.push(nums[i]);
        }
    }
    return negativeNumsArray;
}
...
function onlyPositiveEvens(nums) {
    var positiveEvenNumsArray = [];
    for (var i=0; i < nums.length; i++) {
        if (nums[i]%2 === 0 && nums[i] > 0) {
            positiveEvenNumsArray.push(nums[i]);
        }
    }
    return positiveEvenNumsArray;
}
...
function onlyPositiveOdds(nums) {
    var positiveOddNumsArray = [];
    for (var i=0; i < nums.length; i++) {
        if (nums[i]%2 === 1 && nums[i] > 0) {
            positiveOddNumsArray.push(nums[i]);
        }
    }
    return positiveOddNumsArray;
}
...
function onlyNegativeEvens(nums) {
    var negEvenNums = [];
    for (var i=0; i < nums.length; i++) {
        if (nums[i]%2 === 0 && nums[i] < 0) {
            negEvenNums.push(nums[i]);
        }
    }
    return negEvenNums;
}
...
function onlyNegativeOdds(nums) {
    var negOddNums = [];
    for (var i=0; i < nums.length; i++) {
        if (nums[i]%2 !== 0  && nums[i] < 0) {
            negOddNums.push(nums[i]);
        }
    }
    return negOddNums;
}
