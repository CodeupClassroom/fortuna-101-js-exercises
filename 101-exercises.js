...
var isPositiveEven = function isPositiveEven (x) {
    if(x % 2 == 0 && x > 0) {
        return true;
    } else {
        return false;
...
var isNegativeOdd = function isNegativeOdd (x) {
    if(x % 2 !== 0 && x < 0) {
        return true;
    } else {
        return false;
...
var isNegativeEven = function isNegativeEven (x) {
    if(x % 2 == 0 && x < 0) {
        return true;
    } else {
        return false;
...
function isVowel (x){
    var newL = x.toUpperCase();//this sets it to Uppercase
    if (newL == ('A') || newL == ('U') || newL == ('E') || newL == ('I') || newL == ('O')) {
        return true;
    } else {
        return false;
    }
...
function removeVowels (x) {
    ...
    for (var i = 0;i<x.length;i++){
        if (!isVowel(x.substring(i,i+1))){
            message += x.substring(i,i+1);
        }
    }
    return message;
}
assert(removeVowels("banana"), "bnn", "Exercise 46");//l
...
assert(startsWithVowel("ubuntu"), true, "Exercise 47");//you want to use isVowel(substring(0,1))
...
function endsWithVowel (x) {
    return (isVowel(x.substring(x.length-1,x.length)));
}
...
function startsAndEndsWithVowel (x){
    return (isVowel(x.substring(x.length-1,x.length))) && (isVowel(x.substring(0,1)));
}
...
function firstToLast(sequence){
    var firstElement = sequence.shift();
...
function median(sequence){
    var averageOfEven = (sequence[(sequence.length / 2) - 1] + sequence[sequence.length / 2]) / 2;
    return (sequence.length % 2 === 0)?
        averageOfEven: sequence[Math.floor(sequence.length / 2)];
}
...
function hasEvens(sequence){
    var evenNums = sequence.filter((element) => element % 2 === 0);
...
function countEvens(sequence){
    return sequence.filter((element) => element % 2 === 0).length;
}
...
function hasOdds(sequence){
    var element = sequence.find((element) => element % 2 !== 0);
...
function countNegatives(sequence){
    return sequence.filter((element) => element < 0).length;
}
...
function totalOfBookPrices(objArray){
    var total = 0;
    for(var book of objArray){
        total += book.price;
    }
    return total;
}
...
function getAverageSpentPerItem(shoppingCart){
    var totalCost = 0;
    var totalQuantity = totalNumberOfItems(shoppingCart);
    for(var item of shoppingCart.items){
...
function mostSpentOnItem(shoppingCart){
    var highestCostItem = shoppingCart.items[0];
    var highestCostQuantity = highestCostItem.quantity;
    for(var item of shoppingCart.items){
        var totalItemCost = item.price * item.quantity;
