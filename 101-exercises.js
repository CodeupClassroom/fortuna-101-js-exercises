...
function reverseSign(num) {
    if (isPositive(num)) {return (num *-1)
    } else {return (num /-1)}
}
...
// 1. finds vowels. 2. split string. 3. check each index for vowel. 4. return value.
...
// string in: string with no vowels out.
...
// Function named startWithVowel
// takes string
// read it - probably for each
// if string begins with vowel return true
// if not false
function startsWithVowel(str){
    var toArray = str.split('');
    if(isVowel(toArray[0])){
        return true
    } else {
        return false
    }
...
// string in
// true or false out
// condition 1 : first letter must be a vowel
// condtion 2 : last letter ends with a vowel.


// I will create an if statement where the first condition is the startsWithVowel.

// I will create a function which re joins the array, given that startsWthVowel split them.

// my second else if will be the function endsWithVowel
...
// funciton: first
// split
// move first characther to new Array
// return array
...
// Will return array lenght,
// then I will subtract one.
// Will take that result and return the index of it.
function last(str){
    var counted = str.length - 1;
    return str[counted];
...
// I will write a function that takes in a value
// splits that value**** NO NEED TO SPLIT*****
// counts the value
// subtracts 2 from that count
// returns the origina value - the count - 2
...
// Will count the value
// will will create one variable for the first value
// will create a second variable for the second value
// will return both variables as an array


function firstAndSecond(string){
    var narray = [];
    narray.push(first(string));
    narray.push(second(string));
    return narray;
...
function firstAndLast(string){
    var arrayFL = [];
    arrayFL.push(first(string));
    arrayFL.push(last(string));
    return arrayFL;
...
// 1 will create empty arra// Will insert all values in to array
// // will insert in rear of array
// will return array
...
// 1. re insert sumAll. 2. add a var arrL= .lenght of arr. 3. var arrMean = sum / meanx 4. return arrMean
function mean(array){
...
// 1 Seeking median
// 2 sort array
// 3 if array is odd return number at midpoint ????
// 4 if array is even identify middle 2 numbers and devide my 2

//
function median(arr){
    const mid = Math.floor(arr.length / 2),
        nums = arr.sort((a, b) => a - b);
    var number = arr.length % 2 !== 0 ? nums[mid] : (nums[mid - 1] + nums[mid]) / 2;
...
function maxMinusMin(array){
    ...
    var indexOfMax = sorted.length - 1;
    return (sorted[indexOfMax] - sorted[0]);

}
maxMinusMin([2, 2, 3, 3, 3, 7]);
