...
fruits.splice(fruits.indexOf("strawberry") + 1, 0, "tomato");
...
function quotient(number0, number1) {
    return Math.floor(number0 / number1)
}
...
function remainder(number0, number1) {
    return (number0 % number1)
}
...
function sumOfSquares(number0, number1) {
...
function areaOfRectangle(number0, number1) {
    return (number0 * number1);
}
...
function isVowel(x) {
    return ["A","a","E","e","I","i","O","o","U","u"].indexOf(x) !== -1
}
...
function countVowels(x){
    return (x.match(/[aeiou]/gi) == null) ? 0 : x.match(/[aeiou]/gi).length;
}
...
function startsWithVowel(x) {
    return ["A","a","E","e","I","i","O","o","U","u"].indexOf(x[0]) !== -1
}
...
function endsWithVowel(x) {
    let lstLetter = x.charAt(x.length-1);
    if  (lstLetter == "A"
        || lstLetter == "a"
        || lstLetter == "E"
        || lstLetter == "e"
...
function startsAndEndsWithVowel(x) {
    if(!startsWithVowel(x[0]) && !endsWithVowel(x[0])){
        return false;
    } else {
        return true;
    }
...
// Exercise 57
// Write a function definition named firstAndSecond that takes in sequence and returns the first and second value
// of that sequence as an array
...
// Exercise 59
// Write a function definition named firstToLast that takes in sequence and returns the sequence with the first value
// moved to the end of the sequence.

function firstToLast(x) {
    var firstValue = x.shift();
    x.push(firstValue);
    return x;
}
...
function mean(x) {
    var addAll =  x.reduce((a, b) => a + b, 0);
    return addAll / x.length;
}
...
function median(x) {
    ...
    var mid = x.length / 2;
    return mid % 1 ? x[mid - 0.5] : (x[mid - 1] + x[mid]) / 2;
}
...
function onlyPositiveNumbers(x) {
    return x.filter((element) => element >= 0);
}
...
var hasEvens = function(x) {
    return x.some(function(element) {
        return element % 2 === 0;
    });
};
...
function onlyPositiveEvens(x){
    return x.filter((element) => element % 2 === 0 && element > 0)
}
...
function shortestString(x) {
    let strings = x.filter(x => typeof x === "string").sort((a,b) => a.length - b.length);
    return strings[0];
}
...
function longestString(x) {
    let strings = x.filter(x => typeof x === "string").sort((a,b) => b.length - a.length);
    return strings[0];
}
...
function highestPriceBook(x){
    var mostExpensive = books.reduce((max, book) => max.price > book.price ? max : book);
...
function lowestPriceBook(x){
    var leastExpensive = books.reduce((min, book) => min.price < book.price ? min : book);
...
function numberOfItemTypes(shoppingCart){
    ...
    for(let x of shoppingCart.items){
        uniqueItems.add(x.title);
    }
...
function totalNumberOfItems() {
    ...
    for (var i = 0; i < shoppingCart.items.length; i++) {
        sum = sum + shoppingCart.items[i]["quantity"];
    }
    return sum;
}
...
function getAverageItemPrice(x) {
    var avg = 0;
    for (var i = 0; i < shoppingCart.items.length; i++) {
        avg = avg + (shoppingCart.items[i]["price"] / shoppingCart.items.length);
    }
    return avg;
}
...
function getAverageSpentPerItem(x) {
    ...
    for (var i = 0; i < x.items.length; i++) {
        totalPrice = totalPrice + x.items[i].price * x.items[i].quantity;
        totalQuantity = totalQuantity + x.items[i].quantity;
    }
...
// Exercise 101
// Write a function named mostSpentOnItem that takes in the shopping cart as input and
// returns the object associated with the item that has the highest price*quantity.
// Be sure to do this as programmatically as possible.
// Hint: Similarly to how we sometimes begin a function with setting a variable to zero, we need a starting place:
// Hint: Consider creating a variable that is a object with the keys "price" and "quantity" both set to 0.
...
function mostSpentOnItem(x) {
    ...
    var element = 0;
    for (var i = 0; i < x.items.length; i++) {
        current = x.items[i].price * x.items[i].quantity;
        if (current > highest) {
            highest = current;
            element = i;
        } else {
