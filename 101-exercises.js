...
// The lines below will test your answer. If you see an error, then it means t your answer is incorrect or incomplete.
...
function areaOfRectangle(num, num1) {
    return num * num1;
}
...
function countVowels(value) {
    const vowels = value.match(/[aeiouAEIOU]/g);
...
function removeVowels(value) {
    const novowels = value.replace(/[aeiouAEIOU]/gi, "");
    return novowels;
}
...
function startsWithVowel(string) {
    const vowels = string.match(/[aeiouAEIOU]/);
...
function endsWithVowel(string) {
    return string.search(/[aeuio]$/gi) != -1;
}
...
function firstToLast(sequence) {
    var array = sequence;
    var arrayremoved = array.splice(0, 1);
    array.push(arrayremoved[0]);
    return array;
}
...
function sumAll(sequence) {
    var add = sequence.reduce((a, b) => a + b, 0);
    return add;
}
...
function median(sequence) {
    var median = 0, count = sequence.length;
    sequence.sort();
    if (count % 2 === 0) {
        median = (sequence[count / 2 - 1] + sequence[count / 2]) / 2;
        return median;
    } else {
        median = sequence[(count - 1) / 2];
        return median;
    }
}
...
function maxMinusMin(array) {
    array.sort()
    var minus = array[array.length - 1] - array[0];
    return minus;
}
...
function productOfAll(sequence) {
    var mult = sequence.reduce((a, b) => a * b, 1);
    return mult;
}
...
function onlyPositiveEvens (sequence) {
    ...
    for (var i = 0; i < sequence.length; i++) {
        if (sequence[i] > 0 && sequence[i] % 2 ===0) {
            positivesevens.push(sequence[i]);
        }
    }
...
function shortestString (arr) {
    var strings = arr.sort((a, b) => a.length - b.length);
    return strings[0];
}
...
function longestString (arr) {
    var strings = arr.sort((a, b) => b.length - a.length);
    return strings[0];
}
...
function flatten (arry) {
    var newArray = Array.prototype.concat.apply([], arry);
    return newArray;
}
...
function getPaperTitle (turkeyPaper) {
    return turkeyPaper.title;
}
...
function getYearPublished (turkeyPaper) {
    return turkeyPaper.year_published;
}
...
function totalOfBookPrices (array) {
    var bookprices = 0;
    for (var i = 0; i < array.length; i++)
        bookprices = bookprices + array[i].price;
    return bookprices;
}
// const totalOfBookPrices = arr => arr.reduce((a,b) => a + b.price, 0); another way to do it
...
function getAverageBookPrice (array) {
    var bookprices = 0;
    for (var i = 0; i < array.length; i++)
        bookprices += (array[i].price)/array.length;
    return bookprices;
}
...
function numberOfItemTypes (shoppingCart) {
    var unique = shoppingCart.items.filter((v, i, a) => a.indexOf(v) === i);
...
function getAverageItemPrice(shoppingCart) {
    var sum = 0;
    for (var i = 0; i < shoppingCart.items.length; i++)
        sum = sum + shoppingCart.items[i].price;
...
function mostSpentOnItem(cart) {
    ...
    for(var i = 0; i < cart.items.length; i++) {
        if((cart.items[i].price * cart.items[i].quantity) > totalSpent) {
            totalSpent = (cart.items[i].price * cart.items[i].quantity);
            item = cart.items[i].title;
        }
    }
    return cart.items.find(obj => obj.title === item);
}assert(
