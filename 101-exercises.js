...
function sumOfSquares (numA, numB) {
...
function areaOfRectangle (numA, numB) {
    return (numA * numB)
}
...
function endsWithVowel (string) {
    return string.search(/[aeiou]$/gi) != -1;
}
...
function sumAll (sequence) {
    var array = sequence.reduce ((a,b) => a + b, 0);
    return array
}
...
function productOfAll (sequence) {
    var times = sequence.reduce ((a,b) => a * b, 1);
    return times
}
assert(productOfAll([1, 2, 3]), 6, "Exercise 64");
assert(productOfAll([3, 4, 5]), 60, "Exercise 64");ssert(productOfAll([2, 2, 3, 0]), 0, "Exercise 64");
...
function shortestString (array){
    var sortedArray = array.sort((a,b) => a.length - b.length);
...
function longestString (array){
    var longSort = array.sort((a,b) => b.length - a.length);
    return longSort[0]
}
...
function numberOfItemTypes (ShoppingCart){
    var uniqueItems = shoppingCart.items.filter((v,i,a) => a.indexOf(v) === i);
...
function mostSpentOnItem (shoppingCart){
    ...
    var item;
    for (var i = 0; i < shoppingCart.items.length; i++) {
        if((shoppingCart.items[i].price * shoppingCart.items[i].quantity) > totalSpent){
            totalSpent = (shoppingCart.items[i].price * shoppingCart.items[i]. quantity);
            item = shoppingCart.items[i].title;
        }
    }
    return shoppingCart.items.find(obj => obj.title === item);
}
