...
function sumOfSquares(y, x) {
    var squares = Math.pow(x, 2) + Math.pow(y, 2);
    return squares;
}
...
function firstAndSecond(str) {
    ...
    var firstItem = first(str);
    var secondItem = second(str);
    var firstSecond = [firstItem, secondItem];
...
function firstAndLast(str) {
    ...
    var firstItem = first(str);
    var lastItem = last(str);
    var firstLast = [firstItem, lastItem];
...
function maxMinusMin(str){
    ...
    var min = Math.min.apply(null, str);
...
function getSmallestNumber(str){
...
function onlyOddNumbers(str){
    ...
    return str.filter((item) => Math.abs(item) % 2 === 1);
...
function onlyPositiveNumbers(str){
    ...
    return str.filter((item) => item > 0 && item % parseInt(item) === 0 );
...
function onlyNegativeNumbers(str){
    ...
    return str.filter((item) => item < 0 && item % parseInt(item) === 0 );//filter through array items, return only the negative numbers
...
function hasEvens(str){

    return str.some(item => item % 2 === 0 );//check if an item in the array passes test and return boolean value
...
function countEvens(str){

    return str.filter(item => (item % 2 === 0)).length; //filter the evens and return the length of the occurences, the count
...
function countNegatives(str){

    return str.filter(item => (item < 0)).length; //filter the evens and return the length of the occurences, the count
...
function onlyPositiveEvens(str){
    ...
    return str.filter((item) => item > 0 && item % 2 === 0 );//filter through array items, return only the positive even numbers
...
function onlyPositiveOdds(str){

    return str.filter((item) => item > 0 && item % 2 !== 0 );//filter through array items, return only the positive odd numbers
...
function onlyNegativeEvens(str){

    return str.filter((item) => item < 0 && item % 2 === 0 );//filter through array items, return only the negative even numbers
...
function onlyNegativeOdds(str){
    ...
    return str.filter((item) => item < 0 && item % 2 !== 0 );//filter through array items, return only the negative odd numbers
...
function shortestString(myArray) {
    var sortedArray = myArray.sort(function(a,b){return a.length-b.length});
...
function longestString(myArray) {
    var sortedArray = myArray.sort(function(a,b){return b.length-a.length});
    return sortedArray[0];
}
...
function getUniqueValues(str) {
    var newString = [];
    str = str.filter(function (x, i, a) {
        if ((a.indexOf(x) == i) === true)
...
function elementsTimesTwo(numArray) {
    var arrayResults = [];
    for (var i=0; i < numArray.length; i++) {
        arrayResults[i] = numArray[i]*2;
    }
    return arrayResults;
}
...
function totalOfBookPrices(input) {
    ...
    for (var i=0; i < input.length; i++) {
        sumTotal = sumTotal + input[i].price;
    }
    return sumTotal;
}
...
function highestPriceBook(input) {
    ...
    var element;
    for (var i=0; i < input.length; i++) {
        if (input[i].price > comparePrice) {
            comparePrice = input[i].price
            element = i;
        } else {
            continue;
        }
...
function lowestPriceBook(input) {
    var comparePrice = input[0].price;
    var element = 0;
    for (var i=0; i < input.length; i++) {
        if (input[i].price <= comparePrice) {
            comparePrice = input[i].price;
...
function totalNumberOfItems(input) {
    ...
    for (var i=0; i < input.items.length; i++) {
        sumOfItems = sumOfItems + input.items[i].quantity;
    }
    return sumOfItems;
}
...
function getAverageItemPrice(input) {
    var numberOfItemTypes = input.items.length;
    var priceSum = 0;
    for (var i=0; i < input.items.length; i++) {
        priceSum = priceSum + input.items[i].price;
    }
    return priceSum/numberOfItemTypes;
...
function getAverageSpentPerItem(input) {
    ...
    var totalQty = 0;
    for (var i=0; i < input.items.length; i++) {
        console.log(input.items[i].price, input.items[i].quantity);
        totalSum = totalSum + (input.items[i].price * input.items[i].quantity);
        totalQty = totalQty + input.items[i].quantity;
    }
...
function mostSpentOnItem(input) {
    ...
    var element = 0;
    for (var i=0; i < input.items.length; i++) {
        current = (input.items[i].price * input.items[i].quantity);
        if (current > highest) {
            highest = current;
            element = i;
        } else {
